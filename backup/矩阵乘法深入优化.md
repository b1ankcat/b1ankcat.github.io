            ...
            for (int jb = 0; jb < N; jb += Nc){
                ...
                for (int i2 = 0; i2 < Mc; i2+=Mr){
                    for (int j2 = 0; j2 < Nc; j2+=Nr){
                        ...
                        double*           a = &mc[i2 * N + j2];
                        matmul_cache_block_kernel<Nr, Mr, Kc, Nc>(c, a, b, N, K);
                    }
                }
            }
        }
...
void compute_row(const simd<T>& a, const simd<T>& b, const simd<T>& r, std::index_sequence<J...>)
{
    (..., (r[J] += a * b[J]));
}
...
/** 读写循环展开 **/
template<soze_t RowIdx, typename T, int WIDTH, size_t... I>
...
{
    store_kernel(c, r, N, std::make_index_sequence<Nrs>{});
}
...
OpenBlas的标准性能为Naive实现的约490.3倍。可以看到，OpenBlas的优化真的很厉害。
